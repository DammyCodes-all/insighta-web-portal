...
export default function ProfilesList() {
  ...
  useEffect(() => {
    ...
    async function loadProfiles() {
      ...
      try {
        const response = await api.get<{ data: Profile[]; meta: ProfilesMeta }>(
        ...
        setError(getApiErrorMessage(caughtError, { defaultMessage: 'Could not load profiles right now.' }))
      } finally {
        if (active) {
          setLoading(false)
        }
    ...
    <section className="space-y-5 rounded-[28px] border border-white/10 bg-slate-950/70 p-5 shadow-[0_24px_60px_rgba(0,0,0,0.3)] backdrop-blur-xl sm:p-6">
      <div className="rounded-[22px] border border-white/10 bg-white/5 p-6">
        ...
        <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-slate-50">Profile list</h2>
